...
function createDeviceIngress({ registry, credentials, pipelines, staleMs=15000, now=Date.now }) {
  ...
  function status(workspaceId) {
    ...
    return { ...packet.public, status:stale?'OFFLINE':'DEVICE_DATA',connection:stale?'OFFLINE':'ONLINE', telemetry:stale?'STALE':'AVAILABLE', safety:stale?'UNKNOWN':packet.public.safety, lastTelemetry:new Date(packet.receivedAtMs).toISOString() };
  }
  function ingest({ headers={}, payload }) {
    ...
    if(!result.ok)return response(result.code==='ENGINE_REJECTED'?409:422,{ok:false,code:result.code,errors:result.errors||result.engineError||null,workspaceId:device.workspaceId,source:device.source});
    ...
    const requestedClassification=String(header(headers,'x-gasguard-data-classification')||'').toUpperCase(),dataClassification=requestedClassification==='SYNTHETIC_HANDSHAKE'?'SYNTHETIC_HANDSHAKE':device.source==='TEST_DEVICE'?'VIRTUAL_TEST_DATA':'DEVICE_DATA';
    const publicState={ workspaceId:device.workspaceId,deviceId:device.deviceId,source:device.source,scenario,dataClassification,ingress:'ACCEPTED',packetStatus:dataClassification==='SYNTHETIC_HANDSHAKE'?'SYNTHETIC HANDSHAKE ACCEPTED':'ACCEPTED',sensors:[...new Set(pipeline.engine.state.readings.filter(item=>item.deviceId===device.deviceId).map(item=>item.sensorId))],safety:analysis.safety,gasPpm:analysis.current.gas.ppm??analysis.current.gas.value };
