...
const mobileEvidence = process.argv.includes('--mobile');
const manifest = JSON.parse(await readFile(new URL(mobileEvidence ? '../docs/ui-review/mobile-app/capture-manifest.json' : '../docs/ui-review/capture-manifest.json', import.meta.url), 'utf8'));
const output = new URL(mobileEvidence ? '../docs/ui-review/mobile-app/screenshots/' : '../docs/ui-review/screenshots/', import.meta.url);
...
function channel(url) {
  ...
  return new Promise((resolve, reject) => { ws.addEventListener('open', () => resolve({ call(method, params = {}, sessionId = null) { const messageId = ++id; return new Promise((res, rej) => { waiting.set(messageId, { resolve:res, reject:rej }); ws.send(JSON.stringify({ id:messageId, method, params, ...(sessionId ? { sessionId } : {}) })); }); }, close() { ws.close(); } })); ws.addEventListener('error', reject, { once:true }); });
}
...
try {
  ...
  const target = await browser.call('Target.createTarget', { url:'about:blank' }); const attached = await browser.call('Target.attachToTarget', { targetId:target.targetId, flatten:true }); const cdp = { call:(method,params={})=>browser.call(method,params,attached.sessionId), close() {} };
  ...
  for (const item of manifest) {
    ...
    await cdp.call('Page.navigate', { url:mobileEvidence ? 'http://localhost:5567/?capture=1' : 'http://localhost:5567/?review=1&capture=1' }); await pause(900);
    const role = item.role; const scenario = sourceMap[item.scenario] || 'normal'; const page = item.page || pageMap[item.pageId] || 'overview';
    await runExpression(cdp, `(()=>{const change=(el,value)=>{el.value=value;el.dispatchEvent(new Event('change',{bubbles:true}))};change(document.querySelector('#view-mode'),'developer');const demo=document.querySelector('[data-navigation-page="demo"]');demo&&demo.click()})()`); await pause(250);
    ...
    await runExpression(cdp, `(()=>{const change=(el,value)=>{el.value=value;el.dispatchEvent(new Event('change',{bubbles:true}))};change(document.querySelector('#view-mode'),'${role}');const target=document.querySelector('[data-navigation-page="${page}"]');if(target)target.click();else if('${page}'==='setup')document.querySelector('#open-setup')?.click();const focus=${JSON.stringify(item.focusId || '')}?document.querySelector('#${item.focusId || ''}'):('${item.pageId}'==='D8'?document.querySelector('#guided-demo'):'${item.pageId}'==='D6'?document.querySelector('#relation-explorer-select'):null);if(focus)focus.scrollIntoView({block:'start'});else window.scrollTo(0,0);if(${JSON.stringify(item.overlay || '')}==='more')document.querySelector('#mobile-more-sheet')?.showModal()})()`); await pause(400);
